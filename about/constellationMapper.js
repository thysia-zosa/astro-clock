...
const stereoProject = (radAngle) => {
  return Math.tan(Math.PI / 4 - radAngle / 2);
};

// const consNamesText = fs.readFileSync("./about/consName.json", "utf8");
// const consName = JSON.parse(consNamesText);
const consName = {
  "And":"Andromeda",
  "Ant":"Antlia",
  "Aps":"Apus",
...
// const signaturesText = fs.readFileSync("./about/signatures.json", "utf8");
...
// const asterismText = fs.readFileSync("./about/asterisms.json", "utf8");
// const asterisms = JSON.parse(asterismText);
const asterisms = {
  "Andromeda":"α-δ-β-γ,β-μ-ν",
  "Aquarius":"ε-β-α-γ-ζ-η-λ-τ-δ-88",
  "Aquila":"γ-α-β,ε-ζ-δ-η-θ,α-δ-λ",
  "Aries":"β-α-41",
  "Auriga":"α-β-θ-!-ι-η-ε-α",
  "Bootes":"η-α-ε-δ-β-γ-ρ-α",
  "Cancer":"α-δ-γ-ι,δ-β",
  "Canis Maior":"β-α-ο-δ-η,δ-σ",
  "Canis Minor":"α-β",
  "Capricornus":"α-β-ψ-ω-ζ-δ-γ-ι-θ-β",
  "Cassiopeia":"β-α-γ-δ-ε",
  "Cepheus":"θ-η-α-β-γ-ι-ζ-α,β-ι",
  "Corona Borealis":"ι-ε-δ-γ-α-β-θ",
  "Corvus":"β-δ-γ-ε-β",
  "Crater":"α-β-γ-δ-α",
  "Cygnus":"κ-ι-δ-γ-ε-ζ,α-γ-η-β",
  "Delphinus":"ε-β-α-γ-δ-β",
  "Draco":"λ-κ-α-ι-θ-η-ζ-χ-ε-δ-ξ-ν-β-γ-ξ",
  "Equuleus":"α-δ-γ",
  "Gemini":"η-μ-ε-α-β-δ-γ-ξ",
  "Hercules":"γ-β-ζ-η-σ-τ-φ,η-π-θ-ι,π-ε-δ-α,ε-ζ",
  "Leo":"ε-μ-ζ-γ-δ-β-θ-α-η-γ",
  "Lepus":"η-ζ-α-μ,α-β-ε,β-γ-δ",
  "Libra":"γ-β-α-σ",
  "Lyra":"α-ζ-β-γ-δ-ζ",
  "Ophiuchus":"η-ν-β-α-κ-δ-ε-ζ-η-θ",
  "Orion":"ι-ε-δ-γ-α-ζ-ε,ζ-κ-β-δ",
  "Pegasus":"α-γ-!-β,ε-θ-ζ-α-β-μ,β-η",
  "Perseus":"η-γ-α-β-ρ,α-δ-ε-ξ-ζ-ο",
  "Pisces":"φ-υ-τ-φ-η-ο-α-ξ-ν-μ-ε-δ-ω-ι-θ-γ-κ-λ-ι",
  "Puppis":"ρ-ξ-π",
  "Sagitta":"α-δ-γ,δ-β",
  "Sagittarius":"τ-σ-π-ξ,σ-φ-λ-x0",
  "Scorpius":"τ-α-σ-β,σ-δ,σ-π",
  "Serpens":"κ-β-α-ε-μ,β-γ,θ-η-ν-ξ",
  "Taurus":"β-ε-η,ε-δ-γ-λ-ο,γ-θ-α-ζ",
  "Triangulum":"α-β-γ-α",
  "Ursa Maior":"η-ζ-ε-δ-γ-β-α-δ",
  "Ursa Minor":"α-δ-ε-ζ-β-γ-η-ζ",
  "Virgo":"α-θ-γ-η-β-ν,γ-δ-ε,θ-ζ"
};

const getConstellation = (sig) => {
  if (!sig) return "Unnamed";
  return consName[sig.substring(sig.length - 3)];
};

const calcStar = (starRow) => {
  const [sig, ra, dec, mag] = starRow;
  let id;
  if (!sig) {
    id = "";
  } else {
    const matches = sig.match(/^\d*(.{3})/);
    id = matches[1].trim()
      ? signatures[matches[1].toLowerCase().trim()]
      : matches[0].trim();
  }
...
const consText = fs.readFileSync("./about/stars.csv", "utf8");
const consParts = consText.split("\n").map((e) => e.split(";"));

const constellations = [];
let unnamedStarIndex = 0;
for (const row of consParts) {
  let name = getConstellation(row[0]);
  let consItem = constellations.find((e) => e.name === name);
  if (!consItem) {
    consItem = { name: name, stars: [] };
    constellations.push(consItem);
  }
  let newStar = calcStar(row); // {id: row[1], mag: row[4], x: row[3] - 400, y: 4000 - row[2]};
  if (!newStar.id || consItem.stars.find((e) => e.id === newStar.id))
    newStar.id = `x${unnamedStarIndex++}`;
  if (row[4]) newStar.name = row[4];
...
for (const [name, value] of Object.entries(asterisms)) {
...
console.log(
  `const constellations = ${JSON.stringify(
    constellations,
    null,
    2
